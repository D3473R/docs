...
Run the following script to install `stash-percona-xtradb` addon as a Helm chart.

```console
curl -fsSL https://github.com/stashed/catalog/raw/{{< param "info.catalog" >}}/deploy/chart.sh | bash -s -- --catalog=stash-percona-xtradb
```
...
```console
curl -fsSL https://github.com/stashed/catalog/raw/{{< param "info.catalog" >}}/deploy/chart.sh | bash -s -- --catalog=stash-percona-xtradb --version=5.7
```
